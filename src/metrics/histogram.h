...
class Histogram {
public:
    static constexpr u32 kBucketCount = 240;
    static constexpr f64 kGrowthFactor = 1.15;
    static constexpr f64 kFirstBound = 1e-6;

    Histogram() : buckets_(kBucketCount) { reset(); }

    static constexpr f64 kNoMinimum = 1e300;
    ...
    void record(f64 value) {
        ...
        f64 current_sum = sum_.load(std::memory_order_relaxed);
        while (!sum_.compare_exchange_weak(current_sum, current_sum + value,
                                           std::memory_order_relaxed)) {
        }
        f64 current_max = max_.load(std::memory_order_relaxed);
        while (value > current_max &&
               !max_.compare_exchange_weak(current_max, value, std::memory_order_relaxed)) {
        }
        f64 current_min = min_.load(std::memory_order_relaxed);
    ...
    void reset() {
        ...
        min_.store(kNoMinimum, std::memory_order_relaxed);
    }

    u64 count() const { return count_.load(std::memory_order_relaxed); }

    f64 sum() const { return sum_.load(std::memory_order_relaxed); }
    ...
    f64 max() const { return max_.load(std::memory_order_relaxed); }

    f64 min() const {
        f64 value = min_.load(std::memory_order_relaxed);
        return value == kNoMinimum ? 0.0 : value;
    }
...
private:
    ...
    std::atomic<f64> max_{0.0};
    std::atomic<f64> min_{kNoMinimum};
};
